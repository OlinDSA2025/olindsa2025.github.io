...
Before we start define P versus NP, we need to talk about what we mean by each of these classes.  First, what is P?  We can think of P as a set of decision problems.  A decision problem is a special case of an algorithm where we are deciding something interest (i.e., the output is a 0 or 1).  We haven't dealt with too many of these problems this semester, but many of the problems we've studies thus far can be cast as decision problems.
...
To get a handle on NP problems, let's consider the knapsack problem.  Given a bunch of weighted items, $w_1, w_2, \ldots, w_n$ determine whether there is a partition of these items into two knapsacks such that the weight of the two knapsacks is identical.  If we were given a possible solution to this problem, we could easily verify if it was true or not (how?).  While we can verify a solution to this problem in polynomial time, we don't yet have a way to generate the solution in polynomial time.  If you could figure out such a solution, you would be a billionaire, win the Turing award, and maybe win a Fields medal.

The knapsack problem is part of a set of problems called [the NP-Complete problems](https://en.wikipedia.org/wiki/List_of_NP-complete_problems).  These are problems that are known to be at least as hard as any of the problems in NP.  We show that they are at least as hard using the idea of a reduction.
